...
def _read_image(ignore_data_errors: bool, image_url: str, use_cv2: bool = True,fromurl: bool=False):
    try:
        if use_cv2:
...
def preprocess(image_url, img_size=640,fromurl: bool=False ):
    img0 = _read_image(
        ignore_data_errors=False, image_url=image_url, use_cv2=True,fromurl=True
    ...
    # cv2.imwrite('commingimg.jpg', img0)
    # old_size = img0.shape[:2] # old_size is in (height, width) format
    # # if old_size[1]>old_size[0]:
    # #     img0=cv2.rotate(img0, cv2.ROTATE_90_CLOCKWISE)
    # old_size = img0.shape[:2]
    # logging.info(f'image shape {old_size}')
    # # ratio = float(img_size)/max(old_size)
    # h=int(img_size*old_size[0]/max(old_size))
    # w=int(img_size*old_size[1]/max(old_size))
    # # new_size = tuple([ ratioh*old_size[0],ratiow*old_size[1]])

    # # new_size should be in (width, height) format

    # im = cv2.resize(img0, (w, h))
    # new_size=tuple([ im.shape[:2][0],im.shape[:2][1]])
    # logging.info(f'image shape {im.shape[:2]}')
    # delta_w = img_size - new_size[1]
    # delta_h = img_size - new_size[0]
    # top, bottom = delta_h//2, delta_h-(delta_h//2)
    # left, right = delta_w//2, delta_w-(delta_w//2)

    # pad=(img_size-im.shape[:2][0], img_size-im.shape[:2][1])

    # color = [114, 114, 114]
    # im2 = cv2.copyMakeBorder(im, top, bottom, left, right, cv2.BORDER_CONSTANT,
    #     value=color)
    # # im2.save('img.jpg')
    # cv2.imwrite('img.jpg', im2)
    # img=cv2.resize( img0, (img_size,img_size),interpolation=cv2.INTER_AREA)
    # logging.info(f'image shape {im2.shape[:2]}')
    # Convert
